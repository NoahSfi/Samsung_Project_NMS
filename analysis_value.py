...
def getAP05(model,all_output_dict,img,resFilePath,cocoApi,catIds,catStudied,modelPath,number_IoU_thresh = 50):
    ...
    for iou in tqdm(iou_thresholdXaxis,desc = "progressbar IoU Threshold"):
        ...
        if computeInstances:
            ...
            instances = [int(cocoEval.eval["instances"][0][i]) for i in range(4)]
    ...
    with open("{}/class_value_AP/{}.json".format(modelPath,catStudied), 'w') as fs:
        json.dump({"iou threshold": list(iou_thresholdXaxis),"AP[IoU:0.5]":AP,"False Negatives":FN,"number of instances":instances,"instances_not_ignored":int(instances_non_ignored)}, fs, indent=1)
